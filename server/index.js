...
// ── Force-reseed onboarding user (wipes + rebuilds their data) ───────────────
app.post('/api/admin/reseed-onboard-user', async (req, res) => {
  if (req.headers['x-migrate-key'] !== 'erebus-migrate-2026') return res.status(403).end();
  try {
    const onboardRes = await pool.query("SELECT id FROM users WHERE email='anthony@clara-eu.co'");
    if (!onboardRes.rows.length) return res.status(404).json({ error: 'Onboard user not found' });
    const uid = onboardRes.rows[0].id;

    // Wipe in reverse FK order
    await pool.query('DELETE FROM dive_photos WHERE user_id=$1', [uid]);
    await pool.query('DELETE FROM dive_trip_logs WHERE trip_id IN (SELECT id FROM dive_trips WHERE user_id=$1)', [uid]);
    await pool.query('DELETE FROM dive_trips WHERE user_id=$1', [uid]);
    await pool.query('DELETE FROM dive_log_buddies WHERE dive_log_id IN (SELECT id FROM dive_logs WHERE user_id=$1)', [uid]);
    await pool.query('DELETE FROM dive_log_gases WHERE dive_log_id IN (SELECT id FROM dive_logs WHERE user_id=$1)', [uid]);
    await pool.query('DELETE FROM dive_logs WHERE user_id=$1', [uid]);
    await pool.query('DELETE FROM certification_images WHERE certification_id IN (SELECT id FROM user_certifications WHERE user_id=$1)', [uid]);
    await pool.query('DELETE FROM user_certifications WHERE user_id=$1', [uid]);
    await pool.query('DELETE FROM gear_weights WHERE gear_profile_id IN (SELECT id FROM gear_profiles WHERE user_id=$1)', [uid]);
    await pool.query('DELETE FROM gear_cylinders WHERE gear_profile_id IN (SELECT id FROM gear_profiles WHERE user_id=$1)', [uid]);
    await pool.query('DELETE FROM gear_profiles WHERE user_id=$1', [uid]);
    await pool.query('DELETE FROM equipment_inventory WHERE user_id=$1', [uid]);
    await pool.query('DELETE FROM cylinders WHERE user_id=$1', [uid]);
    await pool.query('DELETE FROM compressors WHERE user_id=$1', [uid]);
    await pool.query('DELETE FROM dive_buddies WHERE user_id=$1', [uid]);
    await pool.query('DELETE FROM dive_site_images WHERE dive_site_id IN (SELECT id FROM dive_sites WHERE user_id=$1)', [uid]);
    await pool.query('DELETE FROM dive_sites WHERE user_id=$1', [uid]);

    const path = require('path');
    const { seedOnboardUser } = require('./run-full-migration.js');
    ...
    const courseIdMap = {};
    for (const row of renderCourseRows.rows) courseIdMap[row.name] = row.id;
    const replitToRender = {};
    for (const c of trainingData.courses) {
      if (courseIdMap[c.name]) replitToRender[c.id] = courseIdMap[c.name];
    }

    const log = [];
    const say = (msg) => { console.log('[reseed]', msg); log.push(msg); };
    await seedOnboardUser(pool, uid, onboardData, say, replitToRender);
