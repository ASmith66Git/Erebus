...
function generateNativeHtml(input: DivePlanPdfInput): string {
  const { result, settings, depth, bottomTime, gases, userName, themeColor } = input;
  const depthUnit = settings.units === 'metric' ? 'm' : 'ft';
  const themeRgb = themeColor ? colorToRgb(themeColor) : [210, 47, 0];
  const themeHex = `rgb(${themeRgb[0]}, ${themeRgb[1]}, ${themeRgb[2]})`;
  ...
  segments.forEach(seg => {
    const gas = gases.find(g => g.o2Percent === seg.gasMix.o2Percent && g.hePercent === seg.gasMix.hePercent);
    const gasName = gas ? formatGas(gas) : formatGas(seg.gasMix);
    segmentRows += `
      <tr>
        <td>${formatDuration(seg.runTime)}</td>
        <td>${Math.round(seg.endDepth)}${depthUnit}</td>
        <td>${formatDuration(seg.duration)}</td>
        <td>${gasName}</td>
        <td style="text-transform: capitalize">${seg.type.replace('_', ' ')}</td>
      </tr>
    `;
  });
  ...
  return `
    ...
    <head>
      ...
      <style>
        ...
        h1 { color: ${themeHex}; font-size: 20px; margin-bottom: 5px; }
        h2 { color: ${themeHex}; font-size: 14px; margin: 15px 0 8px 0; border-bottom: 1px solid #ddd; padding-bottom: 4px; }
        .subtitle { color: #666; font-size: 11px; margin-bottom: 15px; }
        .summary { display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 15px; }
        .summary-item { background: #f5f5f5; padding: 8px 12px; border-radius: 6px; }
        .summary-label { color: #666; font-size: 10px; }
        .summary-value { font-size: 16px; font-weight: bold; color: ${themeHex}; }
        table { width: 100%; border-collapse: collapse; margin-bottom: 15px; font-size: 11px; }
        th { background: ${themeHex}; color: white; padding: 6px 8px; text-align: left; }
        td { padding: 5px 8px; border-bottom: 1px solid #eee; }
        tr:nth-child(even) { background: #f9f9f9; }
        .settings { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; font-size: 11px; }
        .settings-item { display: flex; justify-content: space-between; padding: 4px 0; border-bottom: 1px solid #eee; }
        .footer { margin-top: 20px; padding-top: 10px; border-top: 1px solid #ddd; color: #999; font-size: 9px; }
      </style>
    </head>
    <body>
      <h1>Dive Plan</h1>
      <div class="subtitle">${depth}${depthUnit} for ${bottomTime} min | Generated by ${userName || 'Diver'} | ${new Date().toLocaleDateString()}</div>
      
      <div class="summary">
        <div class="summary-item">
          <div class="summary-label">Max Depth</div>
          <div class="summary-value">${depth}${depthUnit}</div>
        </div>
        <div class="summary-item">
          <div class="summary-label">Bottom Time</div>
          <div class="summary-value">${bottomTime} min</div>
        </div>
        <div class="summary-item">
          <div class="summary-label">Total Runtime</div>
          <div class="summary-value">${Math.ceil(totalRunTime)} min</div>
        </div>
        <div class="summary-item">
          <div class="summary-label">Deco Time</div>
      ...
      <table>
        <thead>
          <tr><th>Runtime</th><th>Depth</th><th>Duration</th><th>Gas</th><th>Phase</th></tr>
        </thead>
      ...
      <table>
        <thead>
          <tr><th>Gas</th><th>O2</th><th>He</th><th>Switch Depth</th></tr>
        </thead>
        <tbody>${gasRows}</tbody>
      </table>
      ...
      <h2>Settings</h2>
      <div class="settings">
        <div class="settings-item"><span>Algorithm</span><span>${settings.decoModel.toUpperCase()}</span></div>
        <div class="settings-item"><span>GF Low/High</span><span>${settings.gfLow}/${settings.gfHigh}</span></div>
        <div class="settings-item"><span>Last Stop</span><span>${settings.lastStopDepth}${depthUnit}</span></div>
        <div class="settings-item"><span>Stop Size</span><span>${settings.stopSize}${depthUnit}</span></div>
        <div class="settings-item"><span>Descent Rate</span><span>${settings.descentRate}${depthUnit}/min</span></div>
        <div class="settings-item"><span>Ascent Rate</span><span>${settings.ascentRate}${depthUnit}/min</span></div>
      </div>

      <div class="footer">
        This dive plan is for reference only. Always verify calculations and follow safe diving practices.<br>
        Erebus Dive Planner
      </div>
    </body>
    </html>
...
export async function downloadDivePlanPdf(input: DivePlanPdfInput): Promise<void> {
  const { settings, depth, bottomTime } = input;
  const depthUnit = settings.units === 'metric' ? 'm' : 'ft';
  const filename = `dive-plan-${depth}${depthUnit}-${bottomTime}min-${new Date().toISOString().split('T')[0]}`;
