...
- **User Authentication**: JWT-based session management with secure offline caching and background token refresh.
- **Admin Panel**: Provides user management and a development log for administrators.
...
- **Gear Profiles**: Manages diving equipment configurations with support for different tank setups, gas mixes, weighting, and a template system.
- **Weather Forecast**: Integrates 7-day marine and atmospheric weather forecasts based on dive site location.
- **Gas Calculator**: Provides comprehensive gas calculation tools including cylinder browsing, density calculation, fill capacity, gas blending (Mix tab with target mix, residual toggle, and blending sequence), and best mix calculation.
